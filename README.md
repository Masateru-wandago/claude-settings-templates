...
| ファイル | 用途 | 安全度 | 作業効率 |
|---|---|---|---|
| `template-readonly.json` | 読み取り専用（調査・レビュー） | ★★★★★ | ★☆☆☆☆ |
| `template-strict.json` | 厳しめ（本番環境近くでの作業） | ★★★★☆ | ★★☆☆☆ |
| `template-infra-balanced.json` | インフラ作業向けバランス型 | ★★★☆☆ | ★★★☆☆ |
...
```bash
cat template-dev-balanced.json
```
...
```bash
# グローバル設定に適用する例
cp template-dev-balanced.json ~/.claude/settings.json
```
